  ...
  return (
    <div className={`lg:w-[70%] relative ${selectedUser?"flex":"hidden"} lg:flex w-full h-full bg-slate-200 border-l-2 border-gray-300`}>
        ...
        <div className='cursor-pointer'>
            <IoIosArrowRoundBack className='w-[40px] h-[40px] text-white cursor-pointer' onClick={()=>dispatch(setSelectedUser(null))}/>
        </div>
        <div className='w-[50px] h-[50px] overflow-hidden rounded-full flex justify-center bg-white items-center shadow-gray-500 shadow-lg cursor-pointer'>
            <img src={selectedUser?.image || dp} alt="" className='h-[100%] '/>
        </div>
        <h1 className='text-white font-semibold text-[20px]'>{selectedUser?.name || "user"}</h1>
      </div>
      <div className='w-full h-[75%] flex flex-col py-[30px] px-[20px] overflow-auto gap-[20px]'>
          ...
          {messages && messages.filter((mess)=>(mess.sender === userData._id && mess.receiver === selectedUser._id) || (mess.sender === selectedUser._id && mess.receiver === userData._id))
          .map((mess)=>
      ...
      <div className='w-full h-full flex flex-col items-center justify-center'>
        ...
        <form className='w-[95%] lg:w-[70%] h-[60px] bg-[rgb(23,151,194)] shadow-gray-400 shadow-lg rounded-full flex items-center gap-[20px] px-[20px] relative' onSubmit={handleSendMessage}>
          ...
          <RiSendPlane2Fill className='w-[25px] h-[25px] text-white cursor-pointer'/></button>)}
        </form>
      </div>}
